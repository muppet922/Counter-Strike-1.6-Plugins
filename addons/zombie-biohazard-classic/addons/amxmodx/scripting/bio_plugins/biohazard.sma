...
public plugin_precache()
{
	...
	cvar_gamedescription = register_cvar("bh_gamedescription", "Biohazard v2.2")
	cvar_skyname = register_cvar("bh_skyname", "blood_")
...
{
	set_hudmessage(0, 255, 0, 0.01, 0.21, _, 0.2, 0.2)
	...
	for(id = 1; id <= g_maxplayers; id++) if(is_user_alive(id) && !is_user_bot(id) && g_zombie[id])
	{
		...
		if(g_classcount > 1)
			ShowSyncHudMsg(id, g_sync_hpdisplay, " [HP: %0.f] ^n [Class: %s] ^n [%s]", health, g_class_name[class], g_class_desc[class])
		else	
...
public task_initround()
{
	...
	set_dhudmessage(random_num(57, 255), random_num(0, 255), random_num(0, 255), -1.0, 0.2, _, _, _, _, _)
	if(newzombie)
	{
		static name[32]
		get_user_name(newzombie, name, 31)
		
		show_dhudmessage(0,"%L", LANG_PLAYER, "INFECTED_HUD", name)
		ColorChat(0, "%L", LANG_PLAYER, "INFECTED_TXT", name)
	}
	else
	{
		show_dhudmessage(0,"%L", LANG_PLAYER, "INFECTED_HUD2")
