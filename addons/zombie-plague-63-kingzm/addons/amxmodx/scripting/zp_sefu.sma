...
public plugin_init()
{
	...
	cvar[GLOW] = register_cvar("zp_leader_glow", "1");
	cvar[TRAIL] = register_cvar("zp_leader_trail", "1");
	cvar[AURA] = register_cvar("zp_leader_aura", "1");
}
...
public zp_user_infected_post(victim, attacker, nem)
{
...
public check_leader()
{
	if(!zp_has_round_started() || zp_is_survivor_round() || g_rounds == 0 || g_rounds == 1)
	{
		return;
	}
	...
	if(leader != last_leader)
	{
		...
		ColorChat(0, "!4[ZP]!1 Noul lider al acestei runde este!3 %s!1.", name);
...
{
	if(!zp_get_user_survivor(id1) || !zp_get_user_survivor(id2))
	{
